...
/**
 * A concise "what to do" line for a single tile, used by the per-tile explain
 * popup and the end-of-game review. Prefers the cited DNA tool, else a default
 * keyed to the tile type.
 */
export function whatToDo(base: string): string {
  const dna = DNA_CARDS[base];
  if (dna?.tool) return dna.tool;
  const d = TILE_DATA[base];
  return defaultTool(d ? d.type : "redflag");
}
